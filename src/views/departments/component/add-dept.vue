...
export default {
  ...
  data() {
    ...
    const checkNameRepeat = async(rule, value, callback) => {
      // value是部门名 要去和同级部门下的部门去比较 有没有相同的 有相同的 不能过 / 不相同就可以过
      const { depts } = await getDepartments()
      // 去找同级部门下有没有和value相同的数据，找到同级部门的所有的子部门
      let isRepeat = false
      if (this.formData.id) {
        // 有id就是编辑模式，编辑的数据在数据库里有  同级部门下我的名字不能和其他的同级部门的名字重复
        // 先找到同级部门  this.formData.id 就是我的id 我的pid是this.formData.pid
    ...
    const checkCodeRepeat = async(rule, value, callback) => {
      ...
      // 要求编码和所有的部门编码都不能重复 由于历史数据可能没有code，所以加一个强制条件就是value值不为空
    ...
    btnOK() {
      ...
      this.$refs.deptForm.validate(async isOK => {
        if (isOK) {
          if (this.formData.id) {
            // 编辑
            await updateDepartments(this.formData) // 不需要搞pid了
          } else {
            // 这里我们将id设成我的pid
