...
export default {
  ...
  methods: {
    async getRoleList() {
      const { total, rows } = await getRoleList(this.page) // 他要两个参数，我给三个，没事
    ...
    changePage(newPage) {
      // 当前点击的页码newPage
      this.page.page = newPage // 将当前页面赋值给当前的最新页码
      this.getRoleList()
    },
    async deleteRole(id) {
      // 提示
